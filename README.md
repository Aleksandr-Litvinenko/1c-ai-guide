...
**Как подключить AI-агента к «1С:Предприятию» и Bitrix24, не потеряв данные.**
...
## Четыре разобранных подключения

Сценарии взяты из интеграций в моих проектах. Для каждого указано, что подтверждено на деле, а что осталось непроверенным.
...
Подтверждено: приватный live-GET к «1С:УНФ». Запись реализована в рабочем коде, но публично не воспроизводилась.
...
Подтверждено: приватные `crm.lead.add` и контрольный `crm.lead.get` со сверкой записанных полей. Публичный commit описывает архитектуру; актуальный пример переведён на универсальный `crm.item.add`, который нужно проверить на своём портале отдельно.
...
## OData умеет не только читать

Через стандартный OData-интерфейс 1С можно создавать, изменять и удалять объекты и проводить документы. Название MCP tool, системный prompt или скрытая кнопка на стороне клиента не создают границу безопасности.
...
| Другие варианты 1С MCP | [Awesome 1C MCP Servers](https://github.com/Untru/1c-mcp) | Широкий курируемый список, полноту и качество каждого проекта он не гарантирует |
...
14 отобранных проектов. Столбец «Проверка» показывает, что сделано: `Docs` — изучены документация и заявления автора, `Artifact` — скачан и проверен релиз, `CLI smoke` — выполнена безопасная локальная команда, `Live smoke` — ответил реальный endpoint.
