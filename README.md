...
[English summary](README.en.md) · [Как всё проверялось](VERIFICATION.md) · [Каталог v2](catalog/tools.json) · [Выбрать стек](recipes/choose-stack.md) · [Предложить проект](CONTRIBUTING.md)
...
Начните с [read-only аудита](recipes/read-only-business-audit.md). Зафиксируйте эталонный отчёт, контрольные суммы и негативные тесты записи до доступа к рабочим данным.

### AI работает с Bitrix24

Начните с [инструкции Bitrix24](recipes/bitrix24-assistant.md). Разделяйте MCP документации и runtime-коннектор: первый знает методы, второй получает ограниченные права конкретного портала.
...
Версия после повторного аудита — `v0.2`: каталог schema v2, пять smoke/artifact проверок и явные границы доказательств. Следующие задачи опубликованы в [ROADMAP.md](ROADMAP.md).
